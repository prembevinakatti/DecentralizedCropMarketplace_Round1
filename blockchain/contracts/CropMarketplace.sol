...
contract CropMarketplace {
    ...
    function listCrop(
        ...
        emit CropListed(cropCount, _name, _quantity, msg.sender, _image, _harvestDate, false);
    }
    ...
    function getCropsForSale() external view returns (CropWithID[] memory) {
        ...
        for (uint256 i = 1; i <= cropCount; i++) {
            if (crops[i].isVerified && !crops[i].isSold && !auctions[i].isAuction) {
                availableCount++;
            }
        }

        CropWithID[] memory availableCropsForSale = new CropWithID[](availableCount);
        uint256 index = 0;
    ...
    function getCropsForAuction() external view returns (CropWithID[] memory) {
        ...
        for (uint256 i = 1; i <= cropCount; i++) {
            if (auctions[i].isAuction && crops[i].isVerified && !crops[i].isSold) {
                availableAuctionsCount++;
            }
        }

        CropWithID[] memory availableCropsForAuction = new CropWithID[](availableAuctionsCount);
        uint256 index = 0;
    ...
    function addQuality(string memory _quality, uint256 _price, uint256 _cropId)
        external
        onlyAgent
    {
        Crop storage crop = crops[_cropId];
        require(crop.isVerified, "Crop must be verified before setting quality");
  ...
  function startAuction(uint256 _cropId) external  {
        ...
        uint256 auctionEndTime = block.timestamp + 60 * 60; // 
...
function getBiddersForCrop(uint256 _cropId) external view returns (address[] memory, uint256[] memory) {
    Auction storage auction = auctions[_cropId];
    require(auction.isAuction, "This crop is not up for auction");
    ...
    return (bidders, bids);
     ...
     function getCropDetails(uint256 _cropId)
        external
        view
        returns (
            uint256 cropId,
    ...
    function getCropsByAgent(address _agent)
    external
    view
    returns (CropWithID[] memory)
{
    ...
    function finalizeAuction(uint256 _cropId) external onlySeller(_cropId) {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            emit AuctionEnded(_cropId, auction.highestBidder, auction.highestBid);
        } else {
    ...
    modifier onlySeller(uint256 _cropId) {
        require(crops[_cropId].seller == msg.sender, "Only seller can call this");
        _;
    }
